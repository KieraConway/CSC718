...
int main(int argc, char *argv[]) {
    ...
    int gap_Start = 0;				//starting value of gap
    int gap_End = 0;				//ending value of gap
    ...
    for(int i = process.start; (i <= process.end|| !found_Next) && i <= N; i+=2){
        ...
        if (isPrime(i)) {
            ...
            if (i == 3 || curr_Val - prev_Val == 2) {
                ...
                /* Toggle Flag when i Exceeds Section */
                if ( i > process.end){
                    found_Next = true;
                }
                ...
                if(i!=3 && gap_End >= gap_Start && gap_Start != 0){
                    curr_Gap = gap_End - gap_Start;		//calculate gap

                    if (curr_Gap > local_Gap){			//if current gap is larger than local gap 
                        local_Gap = curr_Gap;			//update local gap
                    }
                }
