 ...
 */
{
	...
	semaphore_wait( chopstick[n] );     //KC: GET chopstick on right of philo
    draw_chopstick_up( n );
	semaphore_wait( chopstick[(n+1) % NUM_PHILOSOPHERS] );  //KC: Get chopstick on left
	draw_chopstick_up( (n+1) % NUM_PHILOSOPHERS );
    }
}
 ...
 */
{
    ...
    total_number_of_meals += ( total_hungry_time / 1000.0 );
